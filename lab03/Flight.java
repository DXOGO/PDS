...
public class Flight {
    ...
    public Flight(String code, Plane plane) {
        ...
        this.avl_seats_ex = this.plane.getExecutiveSeats();
        this.avl_seats_t = this.plane.getTuristicSeats();

        this.executive_map = new int[this.plane.getExRows()][this.plane.getExCols()];
        this.touristic_map = new int[this.plane.getTuRows()][this.plane.getTuCols()];
    }
    ...
    public Reservation reserveT(int n_seats) {
        //boolean use_next = false;
        int total_seats = n_seats;
        String[] seats = new String[n_seats];

        int n_rows = this.plane.getTuRows();
        int n_cols = this.plane.getTuCols();
            ...
            if (col == n_cols) { // todos os lugares desta fila estão livres
                if (col >= n_seats) { // pode pôr toda a gente aqui
                    ...
                    for (int j = 0; j < n_seats; j++) {
                        seats[total_seats-n_seats+j] = (row+1+this.plane.getExRows()) + " " + String.valueOf((char)(65 + j));                        
                    ...
                    Reservation r = new Reservation('T', this.reservation_number - 1, total_seats, seats);
                    ...
                    for (int j = total_seats - n_seats; j < total_seats && c < n_cols; j++, c++) {
                        seats[total_seats-n_seats+c] = (row+1+this.plane.getExRows()) + " " + String.valueOf((char)(65 + c));
            ...
            if (c >= n_seats) { // pode pôr toda a gente aqui
                ...
                for (int j = 0; j < n_cols && x < n_seats; j++) {
                    if (this.touristic_map[row][j] == 0) {
                        seats[(total_seats + x) - n_seats] = (row+1+this.plane.getExRows()) + " " + String.valueOf((char)(65 + j));
                        this.touristic_map[row][j] = this.reservation_number;
                        x++;
                    }
                }
    ...
    public Reservation reserveE(int n_seats) {
        int total_seats = n_seats;
        String[] seats = new String[n_seats];

        int n_rows = this.plane.getExRows();
        int n_cols = this.plane.getExCols();
        ...
        for (int row = 0; row < n_rows; row++) { // primeiro tenta pôr toda gente junta ou em filas consecutivas se não couberem todos numa fila
            ...
            if (col == n_cols) { // todos os lugares desta fila estão livres
                if (col >= n_seats) { // pode pôr toda a gente aqui
                    ...
                    Reservation r = new Reservation('E', this.reservation_number - 1, total_seats, seats);
                    this.reservations.put(this.reservation_number-1, r);
                    return r;
                }
                else { // mete parte aqui e parte na próxima fila
    ...
    public boolean cancelRes(int r_number) {
        ...
        if (this.reservations.containsKey(r_number)) {
            Reservation r = this.reservations.get(r_number);
            String [] s = r.getSeats();
            ...
            if (r.getClass_code() == 'T') { // vê qual a classe da reserva
                ...
                for (int i = 0; i < s.length; i++) {
                    ...
                    int row = Integer.parseInt(seat[0]) - this.getPlane().getExRows() - 1;
